...
#############################################################################################
# command to run the program and show the output: python vehicle_detection_main.py imshow  #
# command to run the program and save the output: python vehicle_detection_main.py imwrite #
#############################################################################################
    ...
    with detection_graph.as_default():
        ...
        with tf.compat.v1.Session(graph=detection_graph) as sess: # use this line to run it with TensorFlow version 2.x
            ...
            while cap.isOpened():
                ...
                refresh_passed_vehicle = total_passed_vehicle
                print("PASSED : ",refresh_passed_vehicle)
                if(refresh_passed_vehicle == 3):
                    print("IF : ", refresh_passed_vehicle)
                    # ready_to_stop()
                    ready_to_go()
                    ...
                    print("ELSE : ", refresh_passed_vehicle)
                    stop()
                    # go()
                    # time.sleep(15)
                    refresh_passed_vehicle = 0
